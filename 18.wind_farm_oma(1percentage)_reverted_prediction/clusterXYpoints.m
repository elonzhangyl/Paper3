...
%
%% Code
function [clustersXY] = clusterXYpoints(input,maxdist,minClusterSize,method,mergeflag)
...
for k = 1:numel(pairs(:,1))
    ...
    if d <= maxdist
        ...
        elseif ~ismember(pt1,pointsInClusters(2,:))
            % means pt2 is a member
            id = pointsInClusters(1,pointsInClusters(2,:)==pt2); 
            % find the number of the previous cluster where the point 'pt2' already exists.
            switch ['distance to ',lower(method)]
                case 'distance to centroid'
                    cdist = sqrt( sum((mean(XY(clusters{id},:),1) - XY(pt1,:)).^2) ); 
                    % distance between 'pt1' and cluster centroid.
                case 'distance to geometric median'
                    cdist = sqrt( sum((median(XY(clusters{id},:),1) - XY(pt1,:)).^2) ); 
                    % distance between 'pt1' and cluster median point.
                otherwise
                    cdist = [];
            end
            
            if isempty(cdist) || cdist <= maxdist 
                % add 'pt1' to cluster if within 'maxdist' distance of centroid.
                clusters{id} = [ clusters{id} , pt1 ]; 
                % add 'pt1' to the same cluster, in which 'pt2' is.
                pointsInClusters = [ pointsInClusters , [ id ; pt1 ] ]; 
                % add 'pt1' to list of 'points in clusters'.
            end
